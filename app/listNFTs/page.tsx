  ...
  const fetchUserNFTs = async () => {
    try {
      ...
      for (let i = 0; i < balance; i++) {
        ...
        console.log("tokenId:", tokenId)  // Debugging
        const tokenURI = await DynamicNFTContract.methods.tokenURI(tokenId).call();
        console.log("tokenURI:", tokenURI)  // Debugging
      ...
      <main className="flex-1 container px-4 py-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
            ...
            nfts.map((nft) => (
              ...
              >
                <CardHeader>
                  <CardTitle className="dark:text-white">NFT #{nft.tokenId}</CardTitle>
                </CardHeader>
                <CardContent>
                  {/* <img
                    src={nft.tokenURI}
        ...
        {selectedNft && (
          <Card className="mt-8 dark:bg-gray-800">
            <CardHeader>
              <CardTitle className="dark:text-white">Selected NFT: #{selectedNft.tokenId}</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <Card className="mt-8 dark:bg-gray-800">
          <CardContent>
            <p className="text-center text-muted-foreground dark:text-gray-400">
              Note: Listing or unlisting your NFTs will automatically update the listings on both the Kopli and Sepolia networks.
            </p>
          </CardContent>
        </Card>
      </main>
